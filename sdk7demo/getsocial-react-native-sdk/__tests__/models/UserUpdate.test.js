/* eslint-disable no-undef */
import UserUpdate from './../../models/UserUpdate.js';
const {saveResult} = require('./../utils/TestUtils.test.js');

// Test complete user update
const update = new UserUpdate();
update.displayName = 'bob';
update.avatarUrl = 'url';
update.avatar = 'base64image';
update.publicProperties = {'publickey': 'publicvalue'};
update.privateProperties = {'privatekey': 'privatevalue'};
const json = JSON.stringify(update);
const jsonResult = '{"publicProperties":{"publickey":"publicvalue"},"privateProperties":{"privatekey":"privatevalue"},"displayName":"bob","avatarUrl":"url","avatar":"base64image"}';
test('update.toJSON() result shall be', () => {
    expect(json).toBe(jsonResult);
    saveResult('userupdate', 'userupdate.json', json);
});

// Test display name
const update2 = new UserUpdate();
update2.displayName = 'bob';
const json2 = JSON.stringify(update2);
const jsonResult2 = '{"publicProperties":{},"privateProperties":{},"displayName":"bob"}';
test('update2.toJSON() result shall be', () => {
    expect(json2).toBe(jsonResult2);
    saveResult('userupdate', 'userupdate_displayname.json', json2);
});

// Test avatar url
const update3 = new UserUpdate();
update3.avatarUrl = 'url';
const json3 = JSON.stringify(update3);
const jsonResult3 = '{"publicProperties":{},"privateProperties":{},"avatarUrl":"url"}';
test('update3.toJSON() result shall be', () => {
    expect(json3).toBe(jsonResult3);
    saveResult('userupdate', 'userupdate_avatarurl.json', json3);
});

// Test base64 avatar
const update4 = new UserUpdate();
update4.avatar = 'base64image';
const json4 = JSON.stringify(update4);
const jsonResult4 = '{"publicProperties":{},"privateProperties":{},"avatar":"base64image"}';
test('update4.toJSON() result shall be', () => {
    expect(json4).toBe(jsonResult4);
    saveResult('userupdate', 'userupdate_base64avatar.json', json4);
});

// Test public property
const update5 = new UserUpdate();
update5.publicProperties = {'publickey': 'publicvalue'};
const json5 = JSON.stringify(update5);
const jsonResult5 = '{"publicProperties":{"publickey":"publicvalue"},"privateProperties":{}}';
test('update5.toJSON() result shall be', () => {
    expect(json5).toBe(jsonResult5);
    saveResult('userupdate', 'userupdate_publicpropery.json', json5);
});

// Test private property
const update6 = new UserUpdate();
update6.privateProperties = {'privatekey': 'privatevalue'};
const json6 = JSON.stringify(update6);
const jsonResult6 = '{"publicProperties":{},"privateProperties":{"privatekey":"privatevalue"}}';
test('update6.toJSON() result shall be', () => {
    expect(json6).toBe(jsonResult6);
    saveResult('userupdate', 'userupdate_privateproperty.json', json6);
});
