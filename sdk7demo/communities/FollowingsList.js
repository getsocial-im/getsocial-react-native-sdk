...
import {CurrentUser, Communities, FollowersQuery, User, PagingQuery, FollowQuery, ActivitiesView, ActivitiesQuery, UserId, UserIdList} from './../getsocial-react-native-sdk';
...
export default class FollowingsListView extends Component<Props, State> {
    static navigationOptions = {title: 'Followings'};
    static query: UsersQuery;
    ...
    loadFollowings = async () => {
      showLoading();
      Communities.getUsers(new PagingQuery(FollowingsListView.query)).then((result) => {
        hideLoading();
    ...
    loadCurrentUser() {
      GetSocial.getCurrentUser().then((currentUser) => {
        this.setState({ currentUserId: currentUser.id }, this.loadFollowings);
      });
    }
