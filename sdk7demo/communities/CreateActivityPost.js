...
export default class CreateActivityPost extends Component<Props, State> {
  static navigationOptions = {title: 'Create Activity Post'};
    ...
    createPost = async () => {
      ...
      const query = CreateActivityPost.activityTarget == undefined ? ActivitiesQuery.timeline(): ActivitiesQuery.inTopic(CreateActivityPost.activityTarget.getTargetId()); 
      
      if (content.button == null && (content.attachments == null || content.attachments.length == 0) && content.text == null) {
    ...
    render() {
      return (
        <ScrollView style={{flex: 1, padding: 10}}  keyboardDismissMode='on-drag' removeClippedSubviews={false} keyboardShouldPersistTaps='always'>
          ...
          <View style={CreateActivityPostStyle.sectionTitleRow}>
            <View style={{flex: 1, justifyContent: 'center'}}>
              <Button title='Create Activity' onPress={ () => this.createPost() }/>
            </View>
          </View>
